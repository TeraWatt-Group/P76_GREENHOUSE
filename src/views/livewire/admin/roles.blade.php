<div>
    ...
    <div class="row mb-3">
        ...
        <div class="col-6">
            <div class="d-flex justify-content-end">
                <a role="button" aria-pressed="true" class="btn btn-primary ms-2" role="button" href="{{ route('admin.roles.create') }}" aria-label="{{ __('Add') }}">
                    {{ __('Add') }}
                </a>
            </div>
        </div>
    ...
    <div class="row">
        <div class="col-12">
                ...
                <div class="card-body p-0">
                    ...
                    <x-table>
                        <x-slot name="head">
                            ...
                            <x-table.header class="w-15" sortable wire:click="sortBy('name')" :direction="$sorts['name'] ?? null">{{ __('Назва') }}</x-table.header>
                            <x-table.header class="w-10" sortable wire:click="sortBy('guard_name')" :direction="$sorts['guard_name'] ?? null">{{ __('Группа') }}</x-table.header>
                            <x-table.header>{{ __('Дозволи') }}</x-table.header>
                        </x-slot>
                        <x-slot name="body">
                            @forelse ($rows as $row)
                                <x-table.row>
                                    ...
                                    <x-table.cell><a href="{{ route('admin.roles.edit', $row->id) }}" class="" aria-label="{{ __('Edit') }}">{{ $row->name }}</a></x-table.cell>
                                    <x-table.cell>{{ $row->guard_name }}</x-table.cell>
                                    <x-table.cell>
                                        @foreach ($row->permissions as $permission)
                                            <a href="{{ route('admin.permissions.edit', $permission->id) }}" class="me-1" aria-label="{{ __('Edit') }}">{{ $permission->name }}</a>
                                        @endforeach
                                    </x-table.cell>
                                </x-table.row>
                            @empty
            ...
            <div class="d-flex justify-content-start">
                <div>
                    @unless ($selectAll)
                        <span class="btn btn-link me-3 ps-0">{{ $this->selectedCount }} selected</span>
                        <button wire:click="selectAll" class="btn btn-outline-primary me-2" aria-label="{{ __('Select all') }}">Select all</button>
                    @else
                        <span class="btn btn-link me-3 ps-0">{{ $rows->total() }} selected</span>
    ...
    <form wire:submit.prevent="deleteSelected">
    	...
    	<x-modal.dialog-modal wire:model="showDeleteModal">
    		...
    		<x-slot name="content">{{ __('Delete role?') }}</x-slot>
    		<x-slot name="footer">
